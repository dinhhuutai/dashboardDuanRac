...
import { FaHome, FaClipboardCheck, FaPlusCircle, FaChartBar, FaUser } from "react-icons/fa";
          ...
          <div className={`${barCard} relative h-[62px]`}>
            ...
            <div className="grid grid-cols-5 h-full items-center px-2">
              <div className="flex justify-center">
                <TabItem to={config.routes.bmi} label="Trang chủ" icon={<FaHome />} />
              </div>

              <div className="flex justify-center">
                <TabItem to={config.routes.bmiPlan} label="Lộ trình" icon={<FaClipboardCheck />} />
              </div>
              ...
              <div className="flex justify-center">
                <TabItem to={config.routes.bmiDashboard} label="Thống kê" icon={<FaChartBar />} />
              </div>

              <div className="flex justify-center">
                <TabItem to={config.routes.bmiProfile} label="Hồ sơ" icon={<FaUser />} />
              </div>
            </div>

            {/* CENTER FLOATING BUTTON (nhô ra ngoài bar) */}
            <div className="absolute left-1/2 -translate-x-1/2 -top-6">
              <TabCenter to={config.routes.bmiCheck} label="Đo BMI" icon={<FaPlusCircle />} />
            </div>
          </div>
        </div>
      </div>
