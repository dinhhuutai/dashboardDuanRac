...
/* ===== Hook debounce gọn nhẹ ngay trong file ===== */
...
function HistoryWeigh() {
  ...
  const [date, setDate] = useState(() => new Date().toISOString().slice(0, 10));
  const [data, setData] = useState([]);                 // chỉ dữ liệu 1 trang hiện tại
  const [serverTotal, setServerTotal] = useState(0);    // tổng dòng từ server
  const [summary, setSummary] = useState({ totalRows: 0, totalWeight: 0 });
  ...
  const [missingFilter, setMissingFilter] = useState('all'); // vẫn giữ để lọc nhẹ trên 1 trang
  ...
  // Debounce filter để giảm số lần gọi API
  ...
  // ======= Fetch server-side mỗi khi params đổi =======
  useEffect(() => {
    if (!date) return;
    ...
    const load = async () => {
      ...
      try {
        const res = await http.get(`${BASE_URL}/history/date`, {
          ...
          params: {
            ...
            unitName: debouncedFilters.unitName || '',
            trashName: debouncedFilters.trashName || '',
            workShift: debouncedFilters.workShift || '',
            timeFrom: debouncedFilters.timeFrom || '',
            timeTo: debouncedFilters.timeTo || '',
            disposalDate: debouncedFilters.disposalDate || '',
            // 👉 Nếu backend đã hỗ trợ "missing", gửi thêm:
        ...
        if (err.code !== 'ERR_CANCELED') setError('❌ Lỗi khi tải dữ liệu lịch sử cân');
      } finally {
        setLoading(false);
      }
    };
  ...
  const pageData = useMemo(() => {
    if (missingFilter === 'all') return data;
    return data.filter((item) => {
      const hasDate = Boolean(item.workDate);
      const hasShift = Boolean(item.workShift);
      switch (missingFilter) {
        case 'validOnly':    return hasDate && hasShift;
        case 'missingDate':  return !hasDate;
        case 'missingShift': return !hasShift;
        case 'missingEither':return !hasDate || !hasShift;
        case 'missingBoth':  return !hasDate && !hasShift;
        default: return true;
      }
    });
  }, [data, missingFilter]);
  ...
  const exportToExcel = () => {
    const exportData = pageData.map((item, index) => ({
      ...
      'Mã thùng': item.trashBinCode,
      'Thời gian cân': formatDateTime(item.weighingTime),
      'Ngày đổ': formatVietnamDate(item.workDate),
      'Ca': item.workShift || 'Không ca',
      'Người cân': item.userName || '',
      'Khối lượng (kg)': item.weightKg,
    }));
    // Dòng tổng (toàn bộ theo filter trong ngày) -> dùng summary từ server
    exportData.push({});
    exportData.push({
      STT: '',
      'Tài khoản cân': 'Tổng cộng (toàn filter):',
      'Khối lượng (kg)': summary.totalWeight.toFixed(1),
      'Loại rác': `Lượt cân: ${serverTotal}`,
    });

    const ws = XLSX.utils.json_to_sheet(exportData, { skipHeader: false });
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'BaoCaoCanRac');
    const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
    const blob = new Blob([excelBuffer], { type: 'application/octet-stream' });
    saveAs(blob, `BaoCaoCanRac_${date}_P${currentPage}.xlsx`);
  };
    ...
    <div className="p-4 md:p-6">
      ...
      <div className="bg-white/80 backdrop-blur rounded-2xl border border-slate-200 shadow-sm p-4 md:p-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
          ...
          <div className="col-span-1">
            <label className="block text-xs text-slate-500 mb-1">Từ giờ</label>
            <input
              type="time"
          ...
          <div className="col-span-1">
            <label className="block text-xs text-slate-500 mb-1">Đến giờ</label>
            <input
              type="time"
          ...
          <div className="col-span-1">
            <label className="block text-xs text-slate-500 mb-1">Lọc ngày/ca (trang hiện tại)</label>
            <select
              value={missingFilter}
        ...
        <div className="mt-4 flex flex-wrap gap-3 items-center justify-between">
          <div className="text-sm text-slate-600">
            🔢 <span className="font-semibold">{serverTotal}</span> lượt cân | ⚖️{' '}
            <span className="font-semibold">{summary.totalWeight.toFixed(1)}</span> kg
          </div>

          <div className="flex items-center gap-2">
            <button
              onClick={() => refetchCurrentPage()}
              className="px-4 py-2 text-sm rounded-lg border border-slate-300 bg-white hover:bg-slate-50 active:scale-[.98] transition"
            >
              ↻ Tải lại
            </button>
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-white/80 backdrop-blur shadow-sm overflow-hidden">
                  ...
                  >
                    ...
                    <td className="px-2 py-2">
                      {!item.workDate ? (
                        ...
                        <CellPill variant="ok">{formatVietnamDate(item.workDate)}</CellPill>
                      )}
                    </td>
                    <td className="px-2 py-2">
                      {!item.workShift ? (
        ...
        {/* Pagination (Prev/Next + window ngắn) */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 p-3 border-t border-slate-200 text-sm">
          <div>
            Hiển thị:
            <select
            ...
            <span className="ml-3 text-slate-500">Tổng trang: {totalPages}</span>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {/* Hiển thị tối đa 7 nút trang quanh current */}
            {Array.from({ length: totalPages }).slice(
              Math.max(0, currentPage - 4),
              Math.max(0, currentPage - 4) + Math.min(7, totalPages)
            ).map((_, idx) => {
              const n = Math.max(1, currentPage - 3) + idx;
      ...
      {/* Edit modal (không đổi nhiều, chỉ refetchCurrentPage() sau khi lưu) */}
      {editModalVisible && confirmedData && (
        <motion.div
          className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] p-4"
        ...
        >
          ...
          >
            ...
            <div className="text-sm">
              ...
              <div className="flex items-center gap-6 mb-3">
                <label className="flex items-center gap-2">
                  <input type="radio" name="shift" value="true" checked={isWorkShift === true} onChange={() => setIsWorkShift(true)} />
                  <span>Có ca</span>
                </label>
                <label className="flex items-center gap-2">
                  <input type="radio" name="shift" value="false" checked={isWorkShift === false} onChange={() => setIsWorkShift(false)} />
                  <span>Không ca</span>
                </label>
              </div>
              {isWorkShift ? (
            ...
            <div className="text-sm">
              ...
              <div className="flex items-center gap-6 mb-3">
                <label className="flex items-center gap-2">
                  <input type="radio" name="date" value="true" checked={isWorkDate === true} onChange={() => setIsWorkDate(true)} />
                  <span>Có ngày</span>
                </label>
                <label className="flex items-center gap-2">
                  <input type="radio" name="date" value="false" checked={isWorkDate === false} onChange={() => setIsWorkDate(false)} />
                  <span>Không ngày</span>
                </label>
              </div>
