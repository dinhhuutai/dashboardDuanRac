        ...
        <div className="bg-white/80 backdrop-blur rounded-2xl border border-slate-200 p-4 md:p-5 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {/* Segmented Tabs */}
              <div className="relative bg-slate-100 rounded-xl p-1 border border-slate-200 flex">
                {tabs.map(t => (
                  <button
                    key={t.k}
                    onClick={() => setStatusType(t.k)}
                    className={`relative z-10 px-4 py-2 text-sm rounded-lg transition
                      ${statusType===t.k ? "text-emerald-800 font-semibold" : "text-slate-600 hover:text-slate-800"}`}
                    style={{ minWidth: 110 }}
                  >
                    {t.label}
                    {statusType===t.k && (
