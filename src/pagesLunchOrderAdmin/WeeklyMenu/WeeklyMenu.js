...
//           {!disabled && (
//             <button onClick={onRemove} className="absolute top-1 right-1 px-2 py-1 bg-rose-600 text-white text-xs rounded">✕</button>
//           )}
//         </div>
//       ) : (
...
//         {/* Header */}
// <div className="bg-white/80 backdrop-blur rounded-2xl border border-slate-200 p-4 md:p-6 shadow-sm">
...
//           }}
//           className="flex-1 sm:flex-none text-sm border border-slate-200 rounded-lg px-3 py-2 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500"
//         />
...
//                             <button
//                               onClick={async () => {
//                                 const positions = positionsForType(statusType);
//                                 // xoá hết các vị trí của ngày d theo tab hiện tại
//                                 await Promise.all(
//                                   positions.map(p => http.delete(`${BASE_URL}/api/weekly-menus/${weeklyMenu.weeklyMenuId}/entries`, {
//                                     data: { statusType, dayOfWeek:d, position:p }
//                                   }).catch(()=>{}))
//                                 );
//                                 setBoard(prev => {
//                                   const copy = { ...prev };
//                                   positions.forEach(p => delete copy[`slot-${d}-${p}`]);
//                                   return copy;
//                                 });
//                               }}
...
//                       {loading ? (<><FaSpinner className="inline mr-2 animate-spin" />Đang lưu...</>) : "💾 Lưu thực đơn (tab hiện tại)"}
//                     </button>
//                   </div>
//                 </div>
...
//                       />
//                     </div>
//                     <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-3 max-h-[70vh] overflow-y-auto pr-1">
//                       {foodFiltered.map(food => (<DraggableFood key={food.foodId} food={food} />))}
//                     </div>
//                   </div>
//                 </div>
//               </div>
