...
import { BsStars, BsSnow, BsTree, BsUmbrella } from "react-icons/bs";
...
function ThemeToggle({ value, onChange }) {
  ...
  const btnRefs = useRef({}); // map key -> element
  const [slider, setSlider] = useState({ left: 0, width: 0, height: 0 });

  // đo vị trí/size của nút active
  const updateSlider = () => {
    ...
    >
      ...
      <div
        ...
        style={{
          left: slider.left,
          width: slider.width,
          height: slider.height - 8, // trừ viền (tuỳ chỉnh)
        }}
      />
          ...
          >
            <Icon
              className={`h-4 w-4 transition-transform duration-300 ${
                active ? "scale-110" : "scale-100 opacity-80 group-hover:opacity-100"
              }`}
              style={active ? { color: accent } : undefined}
            />

            {/* viền highlight khi active (giữ nguyên của bạn) */}
            {active && (
              <span
...
function HomeMain() {
  ...
  const [theme, setTheme] = useState("normal"); // mặc định: Hè
