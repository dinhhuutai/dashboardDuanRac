  ...
  return (
    <div className="sticky top-[76px] flex h-[calc(100dvh-76px)] flex-col rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      ...
      <NavSection title="Tổng quan" icon={CheckSquare} defaultOpen>
        <NavItem label="Bảng điều khiển" to={config} />
      </NavSection>

      {/* Nhóm 2: Theo phạm vi tổ chức */}
      <NavSection title="Công việc theo phạm vi" icon={Users} defaultOpen>
        <NavItem label="Công việc của tôi" to={config} />
        <NavItem label="Theo nhóm" to={config} />
        <NavItem label="Theo phòng ban" to={config} />
        <NavItem label="Toàn công ty" to={config} />
      </NavSection>

      {/* Nhóm 3: Chế độ xem */}
      <NavSection title="Chế độ xem" icon={LayoutGrid} defaultOpen>
        <NavItem label="Kanban toàn cục" to={config} />
        <NavItem label="Danh sách nâng cao" to={config} />
      </NavSection>

      {/* Nhóm 4: Báo cáo & thông báo */}
      <NavSection title="Báo cáo & thông báo" icon={BarChart3} defaultOpen>
        <NavItem label="Báo cáo" to={config} />
        <NavItem label="Hộp thư & Thông báo" to={config} />
      </NavSection>
...
function NavItem({ label, to = "#" }) {
  ...
  return (
    <Link
      to={to}
      className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-slate-50 ${
