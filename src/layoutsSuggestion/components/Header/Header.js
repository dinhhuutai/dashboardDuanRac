    ...
    <header className={glassBar}>
      <div className="h-[64px] px-3 sm:px-5 hidden md:flex items-center justify-between gap-3 relative">
        ...
        <div className="flex items-center gap-2 min-w-0">
          <Link to={config.routes.feedback1} className="flex items-center gap-2 min-w-0">
        ...
        {/* CENTER: Desktop Nav (bạn bật lại nếu muốn) */}
        <nav className="hidden md:flex items-center gap-1">
          {/* Ví dụ nếu bạn muốn tab:
          <NavLink to={config.routes.home} end className={navBtn}>Trang chủ</NavLink>
          <NavLink to={config.routes.history} className={navBtn}>Lịch sử cân</NavLink>
          <NavLink to={config.routes.user} className={navBtn}>Thông tin</NavLink>
          */}
        </nav>
          ...
          <div className="hidden md:block relative" ref={wrapRef}>
            ...
            >
              ...
              <span
                ...
                style={{
                  backgroundImage: `url(${user?.avatar || logoFeedback})`,
